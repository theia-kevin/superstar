    ...
    <div class="absolute z-0 h-[600px] xl:h-[900px] inset-0 bg-no-repeat bg-cover bg-left-bottom" style="background-image:url('<?php echo get_field('banner_image')['url']; ?>');">
        <div class="w-full h-full container mx-auto pt-[200px] xl:pt-[300px] px-4 grid grid-cols-12">
            <div class="col-span-12 xl:col-span-5 flex flex-col items-start justify-center">
                <div class="font-jumble text-[54px] xl:text-[72px] text-white uppercase leading-[60px] xl:leading-[80px] animate__animated animate__bounce animate__repeat-3"><?php the_field('banner_title'); ?></div>
                ...
                <a class="flex items-center justify-center font-jumble text-[20px] text-white bg-transparent uppercaseleading-[31px] py-[4px] px-[24px] border-2 border-white rounded-[15px] transition-all duration-300 hover:bg-yellow-01 hover:border-yellow-01 hover:text-purple-01 hover:transform hover:-translate-y-2 hover:translate-x-2 hover:shadow-xl" href="<?php the_field('banner_link_url'); ?>"><?php the_field('banner_link_label'); ?></a>
            </div>
        </div>
    </div>
